...
impl Input {
    ...
    pub fn translate_event(&self, event: CrossTermEvent) -> (KeyCode, ActionChain) {
        match event {
            // search event from keymap
            Key(key) => (
                key.code,
                self.keymap.get(&key.code).cloned().unwrap_or_else(|| {
                    if let KeyCode::Char(ch) = key {
                        vec![Event::EvActAddChar(ch)]
                    } else {
                        vec![Event::EvInputKey(key.code)]
                    }
                }),
            ),
            Resize { .. } => (KeyCode::Null, vec![Event::EvActRedraw]),
            _ => (KeyCode::Null, vec![Event::EvInputInvalid]),
        }
    }
    ...
    pub fn bind(&mut self, key: &str, action_chain: ActionChain) {
        ...
        let _ = self.keymap.remove(&code);
        self.keymap.entry(key.code).or_insert(action_chain);
    }
...
#[rustfmt::skip]
fn get_default_key_map() -> HashMap<KeyCode, ActionChain> {
    let mut ret = HashMap::new();
    ret.insert(KeyCode::Esc,          vec![Event::EvActAbort]);
    // ret.insert(KeyCode::Modifier(ModifierKeyCode::LeftControl),    vec![Event::EvActAbort]);
